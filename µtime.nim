...
proc run_command(cmd: seq[string]) =
  let c_cmd = allocCStringArray(cmd)
  ...
  if pid < 0:
    ...
    discard execvp(c_cmd[0], c_cmd)
    error errno, "cannot run ", cmd[0]
    quit if errno == ENOENT: 127 else: 126
  var status: cint
  if waitpid(pid, status, 0) == -1:
    error errno, "error waiting for child process"
    quit -1
  let stop = getMonoTime()
  deallocCStringArray(c_cmd)
